...
<div class="bg-gray-900 flex items-center justify-center min-h-screen py-12 px-6 custom-bg">
    <div class="bg-white shadow-md rounded-lg w-full max-w-md p-6">
        <h2 class="text-2xl font-semibold text-gray-700 mb-4">Přidat nový recept</h2>
        ...
        <form action="{{ route('recipes.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" id="name" class="w-full px-4 py-3 border rounded-lg" required>
            </div>

            <div class="mb-6">
                <label for="ingredients" class="block text-lg font-medium text-gray-700"> Ingredience (oddělit čárkou)</label>
                <textarea name="ingredients" id="ingredients" rows="5" class="w-full px-4 py-3 border rounded-lg" required></textarea>
            </div>

            <div class="mb-6">
                <label for="servings" class="block text-lg font-medium text-gray-700">Počet porcí</label>
                <input type="number" name="servings" id="servings" class="w-full px-4 py-3 border rounded-lg" min="1" max="99" required>
            </div>
            ...
            <div x-data="{ open: false, selectedAllergens: @json(old('allergens', isset($recipe) ? $recipe->allergens->pluck('id') : [])) }" class="relative w-full mb-6">
                <label for="category_id" class="block text-lg font-medium text-gray-700">Alergeny</label>
                <button type="button" @click="open = !open" class="w-full px-4 py-3 border rounded-lg sm:text-left border-gray-500">
                    Vyberte alergeny
                </button>
                <div x-show="open" @click.away="open = false" class="absolute z-10 mt-1 bg-white border rounded-md w-full ">
                    <div class="p-2 space-y-2">
                        @foreach ($allergens as $allergen)
                        <label class="flex items-center space-x-2 cursor-pointer">
                            <input type="checkbox" :value="{{ $allergen->id }}" @click="if ($el.checked) { selectedAllergens.push({{ $allergen->id }}) } else { selectedAllergens = selectedAllergens.filter(i => i !== {{ $allergen->id }}) }" :checked="selectedAllergens.includes({{ $allergen->id }})" class="form-checkbox h-4 w-4 text-blue-600 rounded" />
            ...
            <div class="mb-6">
                ...
                <textarea name="steps" id="steps" rows="5" class="w-full px-4 py-3 border rounded-lg" required></textarea>
            </div>
